...
createTime: 2025/12/20 02:48:20
permalink: /develop_notes/b0fsvr8k/
---
...
| `--generate-chapters 模式` | mkvmerge(1) 可以自动创建章节。目前支持以下两种模式：<br>• 'when-appending' – 此模式在开始时创建一个章节，在追加文件时创建一个章节<br>• 'interval:time-spec' – 此模式以给定的时间间隔创建一个章节 |
...
当以 JSON 模式标识文件时，现有的"LanguageIETF"轨道头元素将作为 language_ietf 轨道属性输出。
...
在类 Unix 系统上，mkvmerge(1) 使用 setlocale(3) 系统调用，后者又使用环境变量 LANG、LC_ALL 和 LC_CTYPE。
...
mkvmerge(1) 使用确定系统区域设置的默认变量（例如 LANG 和 LC_* 系列）。
